...
**Portray**:
The extension's impersonate: it writes {{user}}'s next main-chat input as an action responding to the world given the present circumstances. Text already in the main-chat send box has already decided what {{user}} does; the turn performs that conduct first, at the same scale and substance, in {{user}}'s established voice. The live exchange is a supporting opinion that tilts how {{user}} acts — never material to restage, summarize, or synthesize into the turn. With nothing in the send box and no exchange, the turn comes from {{user}}'s standing state. Scope is {{user}}'s actions and dialogue only, never other characters. By default the result lands editable in the main-chat input box for the player to send; an option sends it immediately.
