...
**Inner connection**:
The extension's own API, model, and reasoning-level settings, inherited from Copilot and separate from the main chat's connection. The inner life can run on a different model than the simulation; changing the main chat's model never silently changes the Inner Voice. Reasoning level is Unset by default — the active preset's thinking is left alone — or a chosen Off, Low, Medium, High, xHigh, or Max applied only to Inner Voice requests.
